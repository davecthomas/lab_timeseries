...
- A condition gets a band only where it genuinely shifts the interval. Where the condition's *signature* is that a metric stays normal — RDW under thalassemia trait, which makes cells uniformly small — the entry is note-only, and the note points away from the condition. Banding RDW there would explain away the discriminator from iron deficiency
...
- A condition adds a reading; it never edits one. The card reports the latest value against the condition band beside the population flag (`▼ low │ ■ normal for Thalassemia trait`). The flag, its colour, its glyph and the sidebar verdict stay as they were — the same rule that stops a ticked checkbox hiding an abnormality. Both verdicts come from `data.status_in`, so they cannot drift apart
- Clicking a legend entry narrows the current selection to the metrics that condition touches. It never charts the condition's whole set, which would pull in results nobody asked for, and with no overlap the selection stays as it is. Like the out-of-range tile it routes through the one callback that owns table data and selection, and guards on `ctx.triggered[0]["value"]`: ticking a condition rebuilds the legend, and Dash fires for recreated components
- "Out of range" means *recent* — `data.out_of_range_now` gates `latest_status` on `is_current` (within `RECENT_MONTHS` of the newest draw). `latest_status` alone answers how a metric's own last reading landed, whenever that was, which surfaced seven retired 2019 spellings of MCV and HGB beside this year's. The tile, its click and the out-of-range filter all read the same rule; `build_table_rows` carries `current` on the row so they cannot drift
